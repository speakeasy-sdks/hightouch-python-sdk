...
class SyncRun:
    ...
    a while.
    """
    failed_rows: FailedRows = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('failedRows') }})
    r"""The number of rows that we attempted to sync, but were rejected by the
    destination.
    ...
    successful_rows: SuccessfulRows = dataclasses.field(metadata={'dataclasses_json': { 'letter_case': utils.get_field_name('successfulRows') }})
    r"""The number of rows that were successfully processed by the destination."""
